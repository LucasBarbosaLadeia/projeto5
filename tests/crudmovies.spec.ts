...
test("crud de movies", async ({ page }) => {
  ...
  //listagem
  await expect(page.getByText("Hellboy")).toBeVisible();
  //falha de listagem
  ...
  await page
    .getByLabel("Imagens")
    .fill("https://br.web.img3.acsta.net/pictures/15/10/15/22/24/429658.jpg");
  ...
  await expect(page.getByText("editado")).toBeVisible({ timeout: 20000 });
  await page.locator("button", { hasText: "editar" }).first().click();
